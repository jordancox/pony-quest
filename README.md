...
A fresh illustrated browser adventure: two connected rooms, separate four-frame walking sprites for the pony and Paul, contextual point-and-click controls, inventory puzzles, branching conversation, and a complete opening-chapter ending.
...
Click or tap the ground to walk. Click an object to perform its natural action; the small status strip previews the action on hover. Right-click to examine, or press and hold on touchscreens. Open **Pockets**, select an item, then click its target. Selecting an item closes the tray. Clicking the ground puts the item away and walks.

`H` reveals hotspots, `I` toggles pockets, `E` examines the focused object, and `Escape` puts an item away and closes the tray. Continue speech with the Continue button or Enter/Space when not focused on another control. All objects are keyboard-focusable. The hint button gives a clue for the current puzzle. There is no verb grid.
...
- `game.css`: responsive game cabinet, pocket tray and overlays.
